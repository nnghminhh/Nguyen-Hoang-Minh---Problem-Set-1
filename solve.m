...
classdef solve
    methods(Static)
        ...
        function sol = grow(par)            
            ...
            g_t =  tau_k*kmat*r + tau_n*n*w - delta*tau_k*kmat;
            ...
            v0 = model.utility(c0,g_t,par)./(1-beta); % Guess of value function for each combination of k and A; each row corresponds to a given k and each column corresponds to a given A.
            ...
            while diff > crit && iter < maxiter % Iterate on the Bellman Equation until convergence.
                ...
                for p = 1:klen % Loop over the k-states.
                    for j = 1:Alen % Loop over the A-states.
                        ...
                        %c = ((1-tau_k)*kgrid(p)*(Agrid(j)*alpha*kgrid(p)^(alpha-1)) + (1-tau_n)*n*w - delta*tau_k*kgrid(p) + g_t(p) - i); % Possible values for consumption, c = y-i, given y and i.
                        c = y - i;
                        ...
                        vall = model.utility(c,g_t(p,j),par) + beta*ev; % Compute the value function for each choice of k', given k.
